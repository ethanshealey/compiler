...
void scanner::scan_digit()
{
	...
	string str;
	bool exitLoop = false, isFloat = false, isE = false, isNegE = false;
    while((isdigit(next_char) or next_char == '.' or tolower(next_char) == 'e' or next_char == '+' or next_char == '-') and not exitLoop){
		if(isdigit(next_char)){ // if digit
		
			if(following_char() == '.'){ // if next char is a .
				
				str += next_char; 
				get_char(); 
				if(following_char() == '.'){
					exitLoop = true; // kill loop
				}
				else if(isdigit(following_char())){
					isFloat = true; // trip float flag
					str += next_char; //add period
				}
				else{
					error->flag(current_line_number, current_pos_on_line, 84);
				}
			}
			else{ 
				str += next_char; 
		...
		else if(tolower(next_char == 'e')) { // if find E
			isE = true;
			str += next_char; 
		}
		else if((next_char == '-' or next_char == '+') and tolower(str.back()) == 'e') { // if find +/- after an E
			str += next_char;
		}
		else if((next_char == '-' or next_char == '+') and tolower(str.back()) != 'e') { // if find a +/- not after an E
			error->flag(current_line_number, current_pos_on_line, 74);
		}
		if(not exitLoop) { // continue loop
			get_char();
		}
		else if(exitLoop and isFloat) {
			error->flag(current_line_number, current_pos_on_line, 104);
		}
		if(tolower(next_char) == 'e' and following_char() == '-')
			isNegE = true;
		else if((next_char == '-' and not isE) or (next_char == '+' and not isE))
			exitLoop = true;
    }  
    if(str.back() == '.') { // number ends in a . 
        error->flag(current_line_number, current_pos_on_line, 63);
    }
	else if(not isFloat and isE and isNegE) { // if neg E without float
		error->flag(current_line_number, current_pos_on_line, 67);
	}
    else if (isFloat or isE) { // return a real number
		current_real_value = stof(str);
		current_symbol = new symbol(symbol::real_num);
    } 
	else if (not isFloat or not isE) { // return an int
		current_integer_value = stoi(str); 
